...
- Memory errors: reduce `max_features` in `TfidfVectorizer` or run on a machine with more RAM.
- Slow transformer training: use a smaller model (DistilBERT) or run on a GPU.
- Tokenization differences: ensure the same tokenizer is used during training and inference.

## Extending This Work

- Add data augmentation (back-translation, synonym replacement) for low-resource classes.
- Experiment with class weights or focal loss for imbalanced labels.
- Add a production inference script (`predict.py`) that loads `joblib` artifacts and exposes a simple CLI or REST endpoint.

## Citations & Resources

- scikit-learn: https://scikit-learn.org
- Hugging Face Transformers (optional): https://huggingface.co/transformers/

## License

This repo is provided without warranty. Add your preferred license (e.g., MIT) as needed.

## Contact

- **Author / Maintainer:** (add your name and contact email here)

---

If you'd like, I can also:

- generate a `requirements.txt` based on the notebook,
- add a small `data/sample_news.csv` example, or
- run the notebook to confirm cells execute end-to-end (requires environment setup).
